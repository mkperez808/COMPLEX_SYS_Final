...
def clean(text, utt, lexicon, oov_dict):
	...
	"""
	# print('clean text3', text)
	# text = __can_skip(text)
	# print('clean text2', text)
	if __can_skip(text)[0]==None:
		print(__can_skip(text))
		return __can_skip(text), "0"
	# print('clean text2', text)
	text = __preprocess(text)
	text = __remove_terminators(text, utt)
	#print('post 1st remove term', text)
	text = __separate_blocks(text, utt)
	text = __group_explanations(text, utt)
	text = __remove_terminators(text, utt)
	#print('post 2nd remove term', text)
	#check text if utt in ['tap19a', 'kansas19a', 'thompson11a', 'kansas20a', 'wright70a']
	# print('clean text1', text)
...
def __separate_blocks(text, utt):
	...
	if len(opening) != len(closing):
		with open('/z/mkperez/HD_Project/Aphasia_Bank_data/overlap_log.txt', 'a') as wf:
			wf.write("prob: %s\n" % (utt))
...
def __remove_unused_tokens(text, utt):
	...
	for w in text:
		...
		#Matt edited: Keep unintelligible words and add unclear token instead of return 'None'
		#xxx or www represent unintelligible sound (unc)
		elif w == 'xxx':
			w = '&=unc'
			new_text.append(w)
		elif w == 'www': #matt added
			w = '&=unc'
			new_text.append(w)                       # Unintelligible word
			...
			match = '&=%s' %re.match(r'\[=(.*)\]', w).group(1).split('! ')[-1]
			w = re.sub(r'\[=(.*)\]', match, w)
			new_text.append(w)
...
def __proc_simple_events(text, utt):
	...
	if len(text) == 0 or text[0] is None:
		return text
	EVENT_MAPS = [
	#paralinguistic = laughing, vocalizes, whispers, mumbled, yawning, whispering, volcaizes, exclaiming, wheezing
	#prosodic = coughs, cough, tongue_clicks, clears_throat, laughs, laughs!, glottal, sniffle, sniffles, grunt, laugh!


		#Fillers
		['<LAU>', ['&=laughs', '&=laughs!', '&=laugh!']],
		['<BRTH>', ['&=sniffle', '&=sniffles', '&=clears_throat', '&=tongue_clicks', '&=cough', '&=coughs', '&=grunt', '&=glottal']],


		#errors (all converted to spn in lexicon)
		# ['<SPN>', ['&=clears_throat', '&=tongue_clicks', '&=cough', '&=coughs', \
		#            '&=grunt', '&=glottal']],
		['<unk>', ['&=unk']],
		['<UNC>', ['&=unc']],
		...
		['<VOICED>', ['&=laughing','&=volcalizes','&=yawning', '&=exclaiming', '&=whispering', '&=whispers',\
		'&=mumbled', '&=wheezing', '&=volcaizes']],
...
def __proc_fragments(text, utt):
	""" Process fragments/fillers (start with '&'). We'll treat all fragments as
...
def __proc_oovs(text, utt, lexicon, oov_dict):
	...
	if len(text) == 0 or text[0] is None:
		return text
	new_text = []
	#print('lex', 'SKILLFELLY' in lexicon)
	for w in text:
